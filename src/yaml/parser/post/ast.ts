...
export const createMap = ([pairs]: any) => {
  ...
  const map = [
    {},
    { this: "mapping", keys: {}, comments: [], sequenceLength: 0 },
  ];
...
export const createSequence = (
  [items]: any,
  sequenceKind: "flowSequence" | "blockSequence"
) => {
  ...
  const sequence: any[] = [
    sequenceValues,
    { this: sequenceKind, isIterable: true, comments },
  ];
  return sequence;
};

export const createFlowSequence = (sequence: any) =>
  createSequence(sequence, "flowSequence");
export const createBlockSequence = (sequence: any) =>
  createSequence(sequence, "blockSequence");
...
export const operate = ([lhs, s1, [op], s2, rhs]: TokenList) =>
  fork([lhs, op.text, rhs]);
